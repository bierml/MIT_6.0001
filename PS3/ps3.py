...
def get_word_score(word, n):
    ...
    for symb in word.lower():
        if symb in SCRABBLE_LETTER_VALUES:
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(handsnum):
        ...
        if(flag):
            print("Current hand:",end=' ')
            display_hand(hand)
            ans=str.lower(input("Would you like to substitute a letter? "))
            if(ans=="yes"):
                let=str.lower(input("Which letter would you like to replace: "))
                hand=substitute_hand(hand,let)
                flag = False
        scorem = play_hand(hand,word_list)
        print("----------")
        if(flag1):
            ans=str.lower(input("Would you like to replay the hand? "))
            if(ans=="yes"):
                flag1 = False
                scorec = play_hand(hand,word_list)
                print("----------")
        if(scorec>scorem):
            scorem = scorec
        score += scorem
